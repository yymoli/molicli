...
var count = 0;
// TODO
var sysoutFunction = function (data) {
    count++;
...
exports.run = function (projectName) {
    ...
    fs.pathExists(targetDirPath, (err, exists) => {
        ...
        if (exists) {
            ...
            // 定时输出，证明程序还活着
            var timer = setInterval(sysoutFunction, 1000, "init npm module ...");
            // 默认环境
            const defaults = {
                cwd: targetDirPath,
                env: process.env
            ...
            npmInstall.stdout.on('data', (data) => {
                console.log('npm system out: ${data}');
            });

            npmInstall.stderr.on('data', (data) => {
                console.log(`npm system error: ${data}`);
            });
